...
const WORDS = [
  ...
  { word: "bat", isCorrect: true },
  { word: "hat", isCorrect: true },
  { word: "rat", isCorrect: true },
  { word: "sit", isCorrect: true },
  { word: "hit", isCorrect: true },
  { word: "bit", isCorrect: true },
  { word: "fit", isCorrect: true },
  { word: "cup", isCorrect: true },
  { word: "bus", isCorrect: true },
  { word: "mug", isCorrect: true },
  { word: "rug", isCorrect: true },
...
export default function AdvancedVowelGame() {
  ...
  useEffect(() => {
    if (gameCompleted && isGameOver && !scoreSaved) {
      ...
      // storeGameScore("vowels", "advanced", {...})
    }
  }, [gameCompleted, isGameOver, scoreSaved, score, shortWordsCaught, longWordsCaught]);

  // ✅ Spawn falling words (faster than intermediate)
  useEffect(() => {
    ...
    }, 1000); // ⚡ faster than intermediate (1000ms vs 1300ms)
    return () => clearInterval(interval);
  }, [gameStarted, isPaused, isGameOver, usedWordIds]);

  // ✅ Move words down (faster than intermediate)
  useEffect(() => {
    if (!gameStarted || isPaused || isGameOver) return;
    const loop = setInterval(() => {
      setFallingWords((prev) =>
        prev
          .map((w) => ({ ...w, y: w.y + 14 })) // ⚡ faster fall (14px vs 9px)
          .filter((w) => w.y < height - 150)
      );
    }, 80); // ⚡ faster interval (80ms vs 110ms)
  ...
  const practiceMessage =
    longWordsCaught > shortWordsCaught / 2
      ? "⚠️ Needs more practice with SHORT A, I, U vowels!"
      : "✅ Excellent! Great recognition of short vowels!";
    ...
    >
      ...
      <Modal visible={instructionsVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.instructionsBox}>
            ...
            <Text style={styles.levelBadge}>ADVANCED LEVEL</Text>

            <Text style={styles.instructionsSubtitle}>Short A, I, U Vowel Sounds</Text>
            <Text style={styles.instructionsText}>
              🎯 Goal: Catch words with SHORT A, I, U sounds
            </Text>
            <Text style={styles.instructionsText}>
              ✅ Correct: cat, sit, cup, bus, mug
            </Text>
            <Text style={styles.instructionsText}>
              ❌ Avoid: cake, kite, rope, team, hope
            </Text>
            <Text style={styles.instructionsText}>
      ...
      {gameStarted && !isGameOver && (
        <View style={styles.gameHeader}>
          ...
          <Text style={styles.gameHeaderLevel}>ADVANCED LEVEL</Text>
        </View>
      )}
      ...
      <Modal visible={showPauseMenu} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.pauseBox}>
            ...
            <Text style={styles.pauseLevel}>Advanced Level</Text>
